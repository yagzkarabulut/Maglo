...
const Card = ({ title, value, icon = "balance", active = false, onClick, children, className = "" }) => {
  ...
  return (
    ...
    >
      <span className="font-medium mb-2 flex items-center gap-2 text-base">
        <span
          className={
            `h-10 rounded-full flex items-center justify-center mr-2 bg-gray-200 ${active ? '' : 'text-gray-700'}`
          }
          style={active ? { color: 'oklch(72.3% 0.219 149.579)' } : {}}
        >
          {icons[icon]}
        </span>
        {title}
      </span>
      <span className={`text-3xl font-bold tracking-tight mt-1 ${active ? 'text-white' : 'text-gray-900'}`}>{value}</span>
      {children}
    </div>
  );
}
