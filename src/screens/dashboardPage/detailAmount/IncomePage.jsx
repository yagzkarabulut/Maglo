...
export default function IncomePage() {
  ...
  // Sadece gelirleri filtrele
  const incomes = Array.isArray(state) ? state.filter(item => item.type === "income") : [];
  // Grafik noktasına tıklanınca detay modalı aç
  const handleDotClick = (dot) => {
    if (!dot?.date || !Array.isArray(incomes)) return;
    const details = incomes.filter((item) => item.date === dot.date);
        ...
        <>
          ...
          <div className="flex min-h-screen w-full">
            ...
            <div className="flex-1 min-w-0 w-full p-2 sm:p-4 md:p-8">
              <h2 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4">Gelirler</h2>
              <div className="mb-6">
                <ChartDataProcessor transactions={incomes}>
                  {(chartData) => (
                    <IncomeExpenseChart
                      data={chartData}
